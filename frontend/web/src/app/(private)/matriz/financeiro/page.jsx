...
export default function FinancasMatriz() {
  ...
  const fetchCategorias = async () => {
    try {
      ...
      if (res.status === 401) {
        console.warn("[fetchCategorias] 401 — não autorizado");
        setCategories([]);
        return;
      }
      ...
      const lista =
        body?.dados ?? (Array.isArray(body) ? body : body?.categorias ?? []);
      ...
      const categoriasComSubcategorias = await Promise.all(
        lista.map(async (categoria) => {
          ...
          try {
            ...
            const subRes = await fetchWithAuth(subUrl, {
              method: "GET",
              credentials: "include",
            ...
            const subs =
              subBody?.dados ?? (Array.isArray(subBody) ? subBody : []);
            return {
            ...
            console.warn(
              "[fetchCategorias] falha ao buscar subcategorias",
              err
            );
            return {
              id: String(categoria.id),
  ...
  const fetchContas = async (tipo) => {
    try {
      ...
      const contas =
        body?.dados ?? body?.contas ?? (Array.isArray(body) ? body : []);

      if (tipo === "payable")
        setAccountsPayable(Array.isArray(contas) ? contas : []);
  ...
  const fetchDashboardStats = async () => {
    try {
      const url = `${API_URL}financeiro/dashboard?mes=${selectedMonth}&ano=${selectedYear}`;
      ...
      const normalized = {
        totalReceivable: Number(
          statsObj.totalReceivable ??
            statsObj.totalRecebimentos ??
            statsObj.totalReceivable ??
            0
        ),
        totalPayable: Number(
          statsObj.totalPayable ??
            statsObj.totalPagamentos ??
            statsObj.totalPayable ??
            0
        ),
        ...
        receivablePendingCount: Number(
          statsObj.receivablePendingCount ?? statsObj.recebiveisPendentes ?? 0
        ),
        payablePendingCount: Number(
          statsObj.payablePendingCount ?? statsObj.pendenciasPagaveis ?? 0
        ),
        receivedCount: Number(
          statsObj.receivedCount ?? statsObj.recebidos ?? 0
        ),
      ...
      console.error("[fetchDashboardStats] erro", err);
    }
  };
    ...
    async function fetchAll() {
      ...
      try {
        ...
        await Promise.all([
          fetchCategorias(),
          fetchContas("payable"),
          fetchContas("receivable"),
          fetchDashboardStats(),
        ]);
      } catch (err) {
        console.error("Erro ao carregar dados do financeiro:", err);
        if (mounted) setError("Erro ao carregar dados do financeiro");
      } finally {
  ...
  const getSelectedMonthName = () => {
    const found = months.find((m) => m.value === String(selectedMonth));
    return found ? found.label : String(selectedMonth);
  };
  ...
  const fetchCategoriasPost = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Erro ao buscar categorias: ${response.status} - ${errorText}`
        );
      }
          ...
          <TabsContent value="dashboard" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Selecione o mês e ano para visualizar os dados específicos do
                  período
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-green-600">
                    {formatCurrency(dashboardStats.totalReceivable)}
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-red-600">
                    {formatCurrency(dashboardStats.totalPayable)}
                  </div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-green-600">
                    {formatCurrency(dashboardStats.totalReceived)}
                  </div>
                  <p className="text-xs text-muted-foreground">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Organização atual das suas categorias
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge className="bg-green-100 text-green-800">
                        {
                          categories.filter((cat) => cat.type === "entrada")
                            .length
                        }
                      </Badge>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <Badge className="bg-red-100 text-red-800">
                        {
                          categories.filter((cat) => cat.type === "saida")
                            .length
                        }
                      </Badge>
                    </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {getSelectedMonthName()} de {selectedYear} - Baseado nas
                    contas do período
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      <span className="text-green-600">
                        {formatCurrency(
                          dashboardStats.totalReceived +
                            dashboardStats.totalReceivable
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {dashboardStats.receivedCount} contas
                        </div>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {dashboardStats.receivablePendingCount} contas
                        </div>
                      </div>
                    </div>
                    <hr />
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {dashboardStats.receivedCount +
                            dashboardStats.receivablePendingCount}{" "}
                          contas
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {dashboardStats.paidCount} contas
                        </div>
                      </div>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {dashboardStats.payablePendingCount} contas
                        </div>
                      </div>
                    </div>
                    <hr />
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {dashboardStats.paidCount +
                            dashboardStats.payablePendingCount}
                          contas
                        </div>
                      </div>
                    </div>
