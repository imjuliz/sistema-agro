    ...
    <div className="min-h-screen px-18 py-10 bg-surface-50">
      <div className="flex gap-6">
        <div className="w-80 space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base"><p>Legenda do Status do Estoque</p></CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 bg-green-500 rounded"></div>
                <div>
                  <h5 className="text-sm font-medium">Acima do Mínimo</h5>
                  <p className="text-sm text-muted-foreground">(Estoque &gt; Min + 5)</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 bg-yellow-500 rounded"></div>
                <div>
                  <h5 className="text-sm font-medium">No Mínimo / Perto do Mínimo</h5>
                  <p className="text-sm text-muted-foreground">(Min - 5 a Min + 5)</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 bg-red-500 rounded"></div>
                <div>
                  <h5 className="text-sm font-medium">Abaixo do Mínimo</h5>
                  <p className="text-sm text-muted-foreground">(Estoque &lt; Min - 5)</p>
                </div>
              </div>
            </CardContent>
        ...
        <AlertDialog open={isModalOpen} onOpenChange={(open) => { if (!open) { setIsModalOpen(false); setModalItem(null); } else setIsModalOpen(open); }}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle><p>Registrar movimentação</p></AlertDialogTitle>
