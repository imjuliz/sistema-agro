...
function Calendar({className,classNames,showOutsideDays = true,components: userComponents,...props}) {
  const defaultClassNames = {
    ...
    weekday: "size-14 p-0 text-xs font-medium text-muted-foreground/80",
    ...
    day: "group size-14 pl-5 px-0 text-sm",
  ...
  return (
    <DayPicker showOutsideDays={showOutsideDays} className={cn("w-100 h-105 bg-white/20 p-5 rounded-xl", className)}
  classNames={mergedClassNames} components={mergedComponents} today={new Date()}
  modifiersClassNames={{today: "bg-primary/50 text-white font-semibold rounded-lg", }}
  {...props}/>
  );
}
