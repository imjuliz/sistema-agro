...
export default function FornecedoresCard({ fornecedores = [], contratos = [], pedidos = [], carregando = false }) {
    ...
    // Robust helpers to extract unit/farm name and supplier name from a contrato object
    const getContractUnitName = (c) => {
        if (!c) return '—';
        // fields that clearly represent the unidade (consumer)
        const unitCandidates = [
            ...
            c?.destinoUnidade?.nome,
            c?.unidade?.nomeUnidade,
            c?.nomeUnidade,
            c?.fazenda?.nome,
            c?.nomeFazenda,
        ];
        for (const v of unitCandidates) if (v) return v;

        // some responses might include the unit under different keys
        if (c?.fornecedorUnidade?.nome) return c.fornecedorUnidade.nome; // fallback, but avoid confusing with supplier
        if (c?.fornecedorInterno?.nome) return c.fornecedorInterno.nome; // sometimes returned as fornecedorInterno

        return '—';
    };

    const getContractSupplierName = (c) => {
        if (!c) return '—';
        const supplierCandidates = [
            ...
            c?.fornecedorInterno?.nomeFantasia,
            c?.nomeFornecedor,
        ];
        for (const v of supplierCandidates) if (v) return v;
        return '—';
    };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(() => {
                ...
                const filteredContratos = contratosConsumidor.filter(c => {
                    ...
                    const nomeUnidade = (getContractUnitName(c) || '').toLowerCase();
                    const nomeFornecedor = (getContractSupplierName(c) || '').toLowerCase();
                ...
                return filteredContratos.length === 0 ? (
                    ...
                    filteredContratos.map((c) => {
                        const nomeUnidade = getContractUnitName(c);
                        const nomeFornecedor = getContractSupplierName(c);
                            ...
                            <CardContent>
                                <div className="flex justify-between items-center mb-2">
                                    <div>
                                        ...
                                        <div className="text-sm text-muted-foreground">Fornecedor: {nomeFornecedor}</div>
                                    </div>
                                    <div className="text-right">
    ...
    const CatalogModal = () => {
        // Nome da unidade e do fornecedor usando helpers para formatos variados
        const nomeUnidade = getContractUnitName(selectedContrato);
        const nomeFornecedor = getContractSupplierName(selectedContrato);
