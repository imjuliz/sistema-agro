 ...
 const OrderDetails = ({ order }) => (
     <Dialog>
       ...
       <DialogContent className="max-w-2xl">
         ...
         <div className="space-y-6">
           ...
           {/* Mostrar descrição do pedido (campo simplificado) */}
           {order.description && (
             <div>
           ...
           {/* Se houver itens, mostre-os (compatibilidade com pedidos mais antigos) */}
           ...
           {/* Removed Rejeitar / Confirmar buttons for external pedido details as requested */}
         </div>
       </DialogContent>
     </Dialog>
   );
  ...
  const handleProcess = async (orderId) => {
    try {
      ...
      if (!res.ok) {
        console.error('[handleProcess] resposta inválida:', body);
        return alert(body.erro || body.message || 'Erro ao processar pedido');
      }
      // refresh UI after successful processing
