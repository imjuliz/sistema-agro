...
export function OverviewTab({ fazendaId }) {
    ...
    const carregarDados = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          toast({ title: 'Erro', description: `Erro ao carregar dados da fazenda (status ${response.status})`, variant: 'destructive' });
          return
        }
        ...
        if (unidade) {
          ...
          toast({ title: 'Erro', description: 'Erro ao carregar dados da fazenda.', variant: 'destructive' });
        }
      } catch (error) {
        console.error("Erro ao buscar dados da fazenda:", error)
        toast({ title: 'Erro', description: 'Erro ao buscar dados da fazenda. Verifique a sua conexão.', variant: 'destructive' });
      } finally {
        setCarregando(false)
      }
    }
  ...
  useEffect(() => {
    ...
    async function carregarDadosGerais() {
      ...
      toast({ title: 'Erro', description: 'Erro ao carregar dados gerais', variant: 'destructive' });
  ...
  async function handleCriarDado(e) {
    ...
    try {
      ...
      setDadosErro(null);
      setOpenNovo(false);
      toast({ title: 'Sucesso', description: 'Dado criado com sucesso' });
    } catch (err) {
      console.error("[dados-gerais] criar", err);
      setDadosErro("Erro ao criar dado");
      toast({ title: 'Erro', description: 'Erro ao criar dado', variant: 'destructive' });
    }
  }
  ...
  async function handleSalvarEdicao(id) {
    try {
      ...
      cancelEdit();
      setDadosErro(null);
      toast({ title: 'Sucesso', description: 'Dado atualizado' });
    } catch (err) {
      console.error("[dados-gerais] atualizar", err);
      setDadosErro("Erro ao atualizar dado");
      toast({ title: 'Erro', description: 'Erro ao atualizar dado', variant: 'destructive' });
    }
  }
  ...
  async function handleExcluir(id) {
    try {
      ...
      toast({ title: 'Sucesso', description: 'Dado excluído' });
    } catch (err) {
      console.error("[dados-gerais] excluir", err);
      setDadosErro("Erro ao excluir dado");
      toast({ title: 'Erro', description: 'Erro ao excluir dado', variant: 'destructive' });
    }
  }
