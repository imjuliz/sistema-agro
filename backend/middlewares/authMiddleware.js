...
export const auth = (perfisPermitidos = []) => {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (decoded.tokenVersion !== user.tokenVersion) { return res.status(401).json({ mensagem: 'Token inválido (revogado)' }); }


      if (perfisPermitidos.length > 0 && !perfisPermitidos.includes(user.perfil.nome)) { return res.status(403).json({ mensagem: 'Acesso negado para este perfil' }); }

      req.usuario = { id: user.id, email: user.email, perfil: user.perfil };
      next();
    }
    catch (err) { return res.status(403).json({ mensagem: 'Token inválido' }); }
  };
};
